...
library(reshape);
```
De nos jours, quand on pense a l'éducation et au passé, on pense souvent que la population était moins éduqué ou bien même que les femmes l'étaient moins. Aujourd'hui, à l'aide de données de l'insee nous allons vérifier ces informations et essayer de comprendre un peu pourquoi à l'aide de simples conclusions.
...
tableeff  %>% ggplot(aes(x=annee,y=value,colour=dip)) + geom_point() +geom_line() + xlab(label = "Année") + ylab(label = "Individus")
...
Nous avions au début parlé d'une differenciation entre homme et femme, nous allons donc examiner si les non-diplômés varient entre hommes et femmes
```{R}
tablepays %>% filter(dip=="aucun") %>% ggplot(aes(x=annee,y=value,color=sexe,linetype=age, shape=age)) + geom_line() + geom_point() + xlab(label = "Année") + ylab(label = "Individus")

```

On remarque grâce à ce graphique le peu de difference dans la variation de l'éducation entre les genres. On remarque également que la population en général a une tendance a s'éduquer de plus en plus. Logiquement, le nombre de diplomés devrait augmenter, nous allons donc examiner cela :
Prenons l'examen des gens ayant au moins un BEP


```{R}
tablepays %>% filter(dip!="aucun") %>% group_by(annee,sexe,age) %>% summarize(value=sum(value)) %>% ggplot(aes(x=annee,y=value,color=sexe,linetype=age, shape=age)) + geom_line() + geom_point() + xlab(label = "Année") + ylab(label = "Individus non dîplomé")



```

Avec ce graph, on peux clairement constater que les femmes ont effectuées un réel progrès au cours des années. Cependant, nous avons ici un effectif, et nous ne pouvons confirmer donc que le taux des femmes était initialement inférieur et viens ensuite égaler les hommes, nous allons donc comparer les pourcentages par diplôme, on ignorera la variable de l'âge pour ce dernier.
...
tableFinale %>% filter(dip!="aucun") %>% group_by(annee,sexe) %>% summarize(pc=sum(pc)) %>% ggplot(aes(x=annee,y=pc,color=sexe)) + geom_line() + geom_point() + xlab(label = "Année") + ylab(label = "Pourcentage de diplomés")

```

Donc grâce à ce graphique, nous pouvons clairement remettre en cause notre hypothèse comme quoi les femmes étaient moins diplomées, elle l'étaient initialement plus et n'ont fais que creuser l'écart au fil des années.
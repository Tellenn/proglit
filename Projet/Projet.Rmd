...
tablepays
```
Voici la taille des echantillon par année : 
```{R}
tableeff <- tablepays %>% group_by(annee,dip)%>% summarize(value=sum(value))
tableeff  %>% ggplot(aes(x=annee,y=value,colour=dip)) + geom_point()
```
 %>% ggplot(aes(x=annee,y=value,colour=sexe)) + geom_point()
```{R}
mutate(tablepays, tot = select(tableeff,value))
```